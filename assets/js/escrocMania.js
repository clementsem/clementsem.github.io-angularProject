...
basket.controller('displayContent',['$rootScope', '$scope', function($rootScope, $scope) {
  ...
  $scope.addBasket = function(id){
    function found(product){
      return product.gameName === $scope.eManiaContent[id].name;
    }
    if($rootScope.basketContent.find(found)){
      for(var i=0; i < $rootScope.basketContent.length; i++){
        if($rootScope.basketContent[i].gameName === $scope.eManiaContent[id].name){
          $rootScope.basketContent[i].gameQty += 1;
          $rootScope.basketContent[i].price += $scope.eManiaContent[id].price;
          $rootScope.total += $scope.eManiaContent[id].price;
        }}
      }else{
        $rootScope.basketContent.push({
          ...
          price: ($scope.eManiaContent[id].price * $scope.eManiaContent[id].quantity),
        });
        $rootScope.total += $scope.eManiaContent[id].price;
      }
    ...
    $scope.increaseQty = function(id) {
      $rootScope.basketContent[id].gameQty += 1;
      $rootScope.basketContent[id].price = ($rootScope.basketContent[id].gamePrice * $rootScope.basketContent[id].gameQty);
    ...
    $scope.decreaseQty = function(id) {
      if ($rootScope.basketContent[id].gameQty > 1) {
        ...
        $rootScope.total -= $rootScope.basketContent[id].price;
